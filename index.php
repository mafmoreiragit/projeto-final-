<?php
	...
	if (isset($_POST['cadastrar'])) {
		...
		$resposta = $produto->insereCrud($array_dados);
		if ($resposta) 
							...
							<form action="index.php" method="POST" class="was-validated">
								...
								<div class="form-group">
								  <label for="preco">Preço (R$):</label>
								   <input type="number" step="any" class="form-control" id="preco" placeholder="00,00" name="preco" required>				  
								</div>
		...
		<tbody>
			<?php
				...
				foreach($resposta as $indice => $valor) {
					...
					echo '<td>R$ ' . htmlspecialchars($valor->preco) . '</td>';
					echo '<td class="text-center">';
					echo '<a href="editar.php?id_produto=' . $valor->id_produto . '" title="Editar"><i class="fa fa-pencil"></i></a> ';
					echo '<a href="excluir.php?id_produto=' . $valor->id_produto . '" title="Excluir"><i class="fa fa-trash-o text-danger"></i></a>';
					echo '</td>';
					echo '</tr>';
				}
			?>
